...
void DLL_Dispose( DLList *list ) {
	DLLElementPtr act = list->firstElement;
	while (act != NULL) {
		DLLElementPtr next = act->nextElement;
		free(act);
		act = next;
	}
...
void DLL_InsertFirst( DLList *list, long data ) {
	DLLElementPtr newNode = malloc(sizeof(struct DLLElement));
	...
	if (list->firstElement != NULL) {
		list->firstElement->previousElement = newNode; // Insertion into nonempty list
		
	} else {
		list->lastElement = newNode; // Insertion into empty list
	}
	list->firstElement = newNode; // Set new node as first
	list->currentLength++;
}
...
void DLL_DeleteFirst( DLList *list ) { 
	if (list->currentLength != 0) {
		DLLElementPtr node = list->firstElement;
...
void DLL_DeleteLast( DLList *list ) {
	...
	if (list->currentLength != 0) {
		DLLElementPtr node = list->lastElement;
...
void DLL_DeleteAfter( DLList *list ) {
	if (list->activeElement != NULL) {
		if (list->activeElement->nextElement != NULL) {
			DLLElementPtr node = list->activeElement->nextElement;
			list->activeElement->nextElement = node->nextElement;

		if (node == list->lastElement ) {
			list->lastElement = list->activeElement;
		
		} else {
			node->nextElement->previousElement = list->activeElement;
		}
		free(node);
		list->currentLength--;
		}
...
void DLL_DeleteBefore( DLList *list ) {
	if (list->activeElement != NULL) {
		if (list->activeElement->previousElement != NULL) {
			DLLElementPtr node = list->activeElement->previousElement;
			list->activeElement->previousElement = node->previousElement;

			if (node == list->firstElement) {
				list->firstElement = list->activeElement;
			} else {
				node->previousElement->nextElement = list->activeElement;
			}
			free(node);
			list->currentLength--;
		}
